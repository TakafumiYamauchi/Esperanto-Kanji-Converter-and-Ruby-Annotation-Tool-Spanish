...
#=====================================================================
# 1) ページ設定 & タイトル (ユーザーが目にするタイトル部分をスペイン語へ)
#=====================================================================
st.set_page_config(
    page_title="Herramienta de generación de archivos JSON para sustituir (caracteres chinos) en textos en Esperanto",
    layout="wide"
)
st.title("Generar un archivo JSON para la sustitución (caracteres chinos) en textos de Esperanto.")
st.write("---")

#=====================================================================
# 2) 概要説明 (使い方) — ユーザーが展開するテキストをスペイン語へ
#=====================================================================
with st.expander("Abrir la explicación de uso", expanded=True):
    st.markdown("""
    #### Introducción
    En esta página se puede generar y descargar
    el archivo JSON (que puede llegar a ser de unos 50 MB)
    utilizado para la sustitución de textos en esperanto
    en la página principal.

    Pasos de uso:
    1. Subir (o usar por defecto) un **archivo CSV** con la correspondencia
       (raíz en esperanto → traducción en español) o caracteres chinos.
    2. Subir (o usar por defecto) un **archivo JSON** que contenga
       las reglas de descomposición de la raíz y/o la cadena de texto a reemplazar.
    3. Pulsar “Crear archivo JSON de sustitución” para
       **descargar** el archivo JSON resultante para la sustitución.

    A continuación hay algunos archivos de ejemplo
    que pueden servir de referencia para el formato
    o para configuraciones personalizadas.
    """)

#=====================================================================
# 3) サンプルファイル一覧 (折りたたみ)
#=====================================================================
with st.expander("Lista de archivos de ejemplo (para descargar)"):
    st.write("#### Lista de archivos de ejemplo")

    st.markdown("""
    **CSV de ejemplo n.º1 (Raíces en esperanto - Traducciones al español con anotaciones ruby)**  
    Contiene correspondencias entre raíces esperanto y traducción al español (posiblemente con anotaciones ruby) en cada línea.  
    Al cargar un CSV en este formato, se genera el archivo JSON para la sustitución.
    """)
    file_path0 = './Appの运行に使用する各类文件/Lista de raíces en esperanto con traducciones en español y anotaciones ruby.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Descargar CSV de ejemplo n.º1 (Raíces esperanto - Traducción al español con anotaciones ruby)",
            data=file,
            file_name="RaícesEsperanto_TraducciónEspañol_conAnotacionesRuby.csv",
            mime="text/csv"
        )

    st.markdown("""
    **CSV de ejemplo n.º2 (Raíces en esperanto - caracteres chinos, versión de Mingeo)**  
    Este CSV asocia cada raíz en esperanto con caracteres chinos, según la propuesta de Mingeo en Zhihu.
    """)
    file_path0 = './Appの运行に使用する各类文件/Mingeo先生版 世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Descargar CSV de ejemplo n.º2 (Raíces esperanto - caracteres chinos de Mingeo)",
            data=file,
            file_name="RaícesEsperanto_CaracteresChinos_Mingeo.csv",
            mime="text/csv"
        )

    st.markdown("""
    **CSV de ejemplo n.º3 (Raíces en esperanto - caracteres chinos)**  
    Este CSV asocia raíces esperanto y caracteres chinos.
    """)
    file_path0 = './Appの运行に使用する各类文件/世界语词根-汉字对应列表.csv'
    with open(file_path0, "rb") as file:
        btn = st.download_button(
            label="Descargar CSV de ejemplo n.º3 (Raíces esperanto - caracteres chinos)",
            data=file,
            file_name="RaícesEsperanto_CaracteresChinos.csv",
            mime="text/csv"
        )

    st.markdown("""
    **JSON de ejemplo n.º1 (Reglas de descomposición de raíces en esperanto, definidas por el usuario)**  
    **Uso**: describe cómo descomponer las palabras esperanto en sus raíces,
    cuándo se añaden sufijos (por ejemplo, terminaciones verbales), y en qué orden.  
    Véanse los comentarios en el archivo para más detalles (ej. `["am", "dflt", ["verbo_s1"]]`).
    """)
    ...
    with open(json_file_path, "rb") as file_json:
        btn_json = st.download_button(
            label="Descargar JSON de ejemplo n.º1 (Descomposición de raíces en esperanto)",
            data=file_json,
            file_name="DescomposiciónDeRaícesEsperanto_configUsuario.json",
            mime="application/json"
        )

    st.markdown("""
    **JSON de ejemplo n.º2 (Cadena de texto de sustitución definida por el usuario)**  
    **Uso**: se puede asignar caracteres chinos u otro formato a palabras específicas,
    además de las reglas de descomposición de raíces del JSON anterior.
    (Normalmente basta editar el CSV y usar el JSON de descomposición de raíces, sin necesitar este fichero.)
    """)
    json_file_path2 = './Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json'
    with open(json_file_path2, "rb") as file_json:
        btn_json = st.download_button(
            label="Descargar JSON de ejemplo n.º2 (Cadena de texto de sustitución definida por el usuario)",
            data=file_json,
            file_name="SustituciónDefinidaPorElUsuario_caracteresChinos.json",
            mime="application/json"
        )

    st.markdown("""
    **Excel de ejemplo n.º1 (4736 raíces en esperanto con traducciones/anotaciones en 14 idiomas)**  
    (Japonés, Chino, Coreano, Inglés, Ruso, Español, Italiano, Francés, Alemán,
    Árabe, Hindi, Polaco, Vietnamita, Indonesio)
    """)
    with open('./Appの运行に使用する各类文件/Kreado de rubenaj komentoj en 14 lingvoj (日本語, 中文, 한국어, English, Русский, español, italiano, français, Deutsch, العربية, हिन्दी, polski, Tiếng Việt, Bahasa Indonesia) respondaj al listo de 4736 Esperant.xlsx', "rb") as file:
        st.download_button(
            label="Descargar Excel n.º1 (Raíces esperanto con anotaciones en 14 idiomas)",
            data=file,
            file_name="RaícesEsperanto_conAnotaciones_14Idiomas.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )

    st.markdown("""
    **Excel de ejemplo n.º2 (Raíces en esperanto - traducción al japonés con nivel de aprendizaje)**  
    **Uso**: si desea personalizar las raíces esperanto a las que se añaden rubíes de traducción.
    Incluye un "nivel de aprendizaje" basado en un diccionario básico de esperanto-japonés.
    """)
    with open('./Appの运行に使用する各类文件/エスペラント語根-日本語訳ルビ対応リスト(習得レベル付き).xlsx', "rb") as file:
        st.download_button(
            label="Descargar Excel n.º2 (Raíces esperanto - traducción al japonés con nivel de aprendizaje)",
            data=file,
            file_name="RaícesEsperanto_JaponésNivelAprendizaje.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
...
#=====================================================================
# ユーザーが選択する出力形式 (GUIに見えるキーをスペイン語へ)
#=====================================================================
options = {
...
selected_display = st.selectbox('Seleccionar el formato de salida:', display_options)
format_type = options[selected_display]

# Ejemplo rápido de formateo
...
st.write("---")
st.markdown("**Texto formateado de ejemplo (vista previa):**")
components.html(apply_ruby_html_header_and_footer(formatted_text, format_type), height=40, scrolling=False)
st.write("---")

#=====================================================================
# ステップ１: CSVファイルを準備 (アップロード or デフォルト) — スペイン語に
#=====================================================================
st.header("Paso 1: Preparar el archivo CSV")
st.markdown(
    """
### Seleccione el **archivo CSV** que contenga las raíces en esperanto y la información de traducción (caracteres chinos o español).

---
    """
)

csv_choice = st.radio("¿Cómo proceder con el archivo CSV?", ("Subir un archivo CSV", "Usar archivo por defecto"))
csv_path_default = "./Appの运行に使用する各类文件/Lista de raíces en esperanto con traducciones en español y anotaciones ruby.csv"

CSV_data_imported = None

if csv_choice == "Subir un archivo CSV":
    st.write("Por favor, suba un archivo CSV (recomendado UTF-8).")
    ...
    if uploaded_file is not None:
        ...
        st.warning("No se ha subido ningún archivo CSV.")
        st.stop()

elif csv_choice == "Usar archivo por defecto":
    try:
        ...
        csv_buffer = StringIO(converted_text)
        CSV_data_imported = pd.read_csv(csv_buffer, encoding="utf-8", usecols=[0, 1])
        st.info("Se utilizará el archivo CSV por defecto.")
    except FileNotFoundError:
        st.error("No se encontró el archivo CSV por defecto. Se interrumpe el proceso.")
        st.stop()

st.write("La carga del archivo CSV se ha completado. Continuemos al siguiente paso.")
st.write("---")

#=====================================================================
# ステップ2: JSONファイル(語根分解法など)を準備 — スペイン語で
#=====================================================================
st.header("Paso 2: Preparar el/los archivo(s) JSON (reglas de descomposición, etc.)")
st.markdown("""
Puede subir o utilizar un archivo JSON por defecto que
contenga **las reglas de descomposición de raíces esperanto** y/o
**los textos de sustitución personalizados**.
Se puede descargar un JSON de ejemplo y modificarlo para sus necesidades.
""")

json_choice = st.radio("1. ¿Cómo proceder con el archivo JSON que define la descomposición de las raíces esperanto?", ("Subir un archivo JSON", "Usar archivo por defecto"))
json_path_default = "./Appの运行に使用する各类文件/世界语单词词根分解方法の使用者自定义设置.json"
custom_stemming_setting_list = None

if json_choice == "Subir un archivo JSON":
    uploaded_json = st.file_uploader("Subir archivo JSON", type=['json'])
    if uploaded_json is not None:
        custom_stemming_setting_list = json.load(uploaded_json)
        st.success("Archivo JSON subido correctamente.")
    else:
        st.warning("No se ha subido ningún archivo JSON.")
        st.stop()
elif json_choice == "Usar archivo por defecto":
    try:
        with open(json_path_default, "r", encoding="utf-8") as g:
            custom_stemming_setting_list = json.load(g)
        st.info("Se utilizará el archivo JSON por defecto.")
    except FileNotFoundError:
        st.error("No se encontró el archivo JSON por defecto.")
        st.stop()

json_choice2 = st.radio("2. ¿Cómo proceder con el archivo JSON que define el texto de sustitución?", ("Subir un archivo JSON", "Usar archivo por defecto"))
json_path_default2 = "./Appの运行に使用する各类文件/替换后文字列(汉字)の使用者自定义设置(基本上完全不推荐).json"
user_replacement_item_setting_list = None

if json_choice2 == "Subir un archivo JSON":
    uploaded_json = st.file_uploader("Subir el segundo archivo JSON", type=['json'])
    if uploaded_json is not None:
        ...
        st.warning("No se ha subido ningún archivo JSON.")
        st.stop()
elif json_choice2 == "Usar archivo por defecto":
    try:
        with open(json_path_default2, "r", encoding="utf-8") as g:
            user_replacement_item_setting_list = json.load(g)
        st.info("Se utilizará el segundo archivo JSON por defecto.")
    except FileNotFoundError:
        st.error("No se encontró el segundo archivo JSON por defecto.")
        st.stop()

st.write("---")

#=====================================================================
# ステップ3: 高度な設定 (並列処理) — スペイン語
#=====================================================================
st.header("Paso 3: Configuración avanzada (procesamiento en paralelo)")
with st.expander("Abrir la configuración para el procesamiento en paralelo"):
    st.write("""
    Aquí se puede especificar el número de procesos que se usarán
    para el procesamiento en paralelo al crear el archivo JSON.
    Si el texto o la cantidad de raíces es muy grande,
    se puede acelerar el proceso usando varios núcleos de CPU.
    """)

    use_parallel = st.checkbox("Utilizar procesamiento en paralelo", value=False)
    num_processes = st.number_input("Número de procesos en paralelo", min_value=2, max_value=6, value=5, step=1)

st.write("### Crear el archivo JSON final para la sustitución (botón)")

#=====================================================================
# Botón para crear archivo JSON de sustitución (ユーザーが押す所)
#=====================================================================
if st.button("Crear archivo JSON para la sustitución"):
    with st.spinner("Generando el archivo JSON de sustitución... Por favor, espere un momento."):
        #-------------------------------------------------------------
        # (1) Abrir datos masivos 'E_stem_with_Part_Of_Speech_list'
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (2) Cargar todas las raíces del esperanto (~11137) en un dict
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (3) Usar CSV_data_imported para aplicar
        #     "Raíz en esperanto → (caracteres chinos o traducción en español)"
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (4) Convertir dict en lista y ordenar
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (5) Crear lista con placeholders para realizar safe_replace()
        #-------------------------------------------------------------
        ...
        #-------------------------------------------------------------
        # (6) Ejecutar parallel_build_pre_replacements_dict
        #-------------------------------------------------------------
        if use_parallel:
            ...
            for i, j in enumerate(E_stem_with_Part_Of_Speech_list):
                ...
                if i % 1000 == 0:
                    ...
                    progress_text.write(f"Procesando {current_count}/{total_items} elementos...")

            progress_bar.progress(100)
            progress_text.write("La tarea más larga ha finalizado al 100%. (Se requieren ~3-4 segundos más.)")

        # ... 中略 (otras uniones, merges, etc.)
        ...
        # JSON文字列にダンプ → ダウンロードボタン
        download_data = json.dumps(combined_data, ensure_ascii=False, indent=2)
        st.success("¡Generación de la lista de sustitución completada!")

        st.download_button(
            label="Descargar la lista final de sustitución (fusión de 3 archivos JSON)",
            data=download_data,
            file_name="ListaFinalSustitución_Fusión3ArchivosJSON.json",
            mime='application/json'
        )